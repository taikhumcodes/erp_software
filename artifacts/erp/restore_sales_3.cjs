...
const targetRegex = /<TableCell>\{new Date\(sale\.saleDate\)\.toLocaleDateString\(\)\}<\/TableCell>[\s\S]*?<DropdownMenuItem onClick=\{\(\) => \{[\s\S]*?window\.open\('\/documents\/sales-invoice\/' \+ sale\.id, '_blank'\);/m;

const replacement = `<TableCell>{new Date(sale.saleDate).toLocaleDateString()}</TableCell>
                    <TableCell>
...
if (targetRegex.test(code)) {
  ...
  console.log('Restored table body! YES!');
} else {
