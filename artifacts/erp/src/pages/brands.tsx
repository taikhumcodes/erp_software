...
interface BrandForm { name: string; nameAr: string; logoUrl: string; isActive: boolean; }
const emptyForm = (): BrandForm => ({ name: '', nameAr: '', logoUrl: '', isActive: true });
...
export default function Brands() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<Brand>>({
    queryKey: ['brands', page, search],
    queryFn: () => api.get(`/brands?${params}`),
  });

  const invalidate = () => qc.invalidateQueries({ queryKey: ['brands'] });

  const createMutation = useMutation({
    mutationFn: (body: BrandForm) => api.post<Brand>('/brands', body),
    onSuccess: () => { invalidate(); closeModal(); toast({ title: t('brand_created') }); },
    onError: (e: Error) => toast({ title: t('error'), description: e.message, variant: 'destructive' }),
  });

  const updateMutation = useMutation({
    mutationFn: (body: BrandForm) => api.put<Brand>(`/brands/${editing!.id}`, body),
    onSuccess: () => { invalidate(); closeModal(); toast({ title: t('brand_updated') }); },
    onError: (e: Error) => toast({ title: t('error'), description: e.message, variant: 'destructive' }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.del(`/brands/${id}`),
    onSuccess: () => { invalidate(); setDeleteTarget(null); toast({ title: t('brand_deleted') }); },
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 justify-between items-center">
        ...
        <button onClick={openCreate} className="bg-primary hover:bg-primary/90 text-primary-foreground font-medium py-2 px-4 rounded-md transition-colors shadow-sm flex items-center gap-2">
          <Plus className="w-4 h-4" />{t('add_brand')}
        </button>
      </div>
            ...
            <tbody className="divide-y">
              ...
              {data?.data.map((brand) => (
                <tr key={brand.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-muted-foreground" dir="rtl">{brand.nameAr || '—'}</td>
                  <td className="px-6 py-4">
                    <StatusBadge active={brand.isActive} t={t} />
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(brand)} title={t('edit')} className="p-1.5 rounded hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors"><Pencil className="w-4 h-4" /></button>
                      <button onClick={() => setDeleteTarget(brand)} title={t('delete')} className="p-1.5 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={modalOpen} onOpenChange={(v) => !v && closeModal()}>
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader><DialogTitle>{editing ? t('edit_brand') : t('add_brand')}</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            <div className="space-y-1.5">
              <label className="text-sm font-medium">{t('name_en')} <span className="text-destructive">*</span></label>
              <input required value={form.name} onChange={f('name')} className="form-input" placeholder="e.g. Hilti" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <input value={form.logoUrl} onChange={f('logoUrl')} className="form-input" placeholder="https://..." dir="ltr" />
            </div>
            {editing && (
      ...
      <ConfirmDialog
        open={!!deleteTarget} title={t('delete_brand_confirm')}
