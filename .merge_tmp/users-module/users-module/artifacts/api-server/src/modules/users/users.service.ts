...
// ─── Actor context (passed from controller from req.user) ─────────────────────
...
export const UsersService = {
  ...
  async create(actor: ActorContext, body: Record<string, unknown>) {
    ...
    if (!canActOn(actor.role, role!)) {
      // OWNER can only be created by another OWNER
      if (role === 'OWNER') {
  ...
  async update(actor: ActorContext, id: string, body: Record<string, unknown>) {
    ...
    return UsersRepository.update(id, { name, nameAr, role, isActive });
  },
  ...
  async resetPassword(actor: ActorContext, id: string, body: Record<string, unknown>) {
    ...
    // Actor must outrank target (unless resetting their own password)
    if (actor.id !== id) {
      if (!canActOn(actor.role, target.role)) {
